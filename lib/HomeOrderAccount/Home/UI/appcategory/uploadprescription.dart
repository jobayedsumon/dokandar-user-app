...
class UploadPrescription extends StatefulWidget {
  ...
  UploadPrescription(this.vendorId);
...
class UploadPrescriptionState extends State<UploadPrescription> {
                  ...
                  Container(
                    child: Column(
                      ...
                      children: [
                        ...
                        BottomBar(
                            text: locale.placeOrderRequestText,
                            onTap: () {
                              // dateTimeSt
                              if ((_image != null && _image.path != null) &&
                                  (addressDelivery != null &&
                                      addressDelivery.addressId != null)) {
                                setState(() {
                                  showDialogBox = !showDialogBox;
                                });
                                hitUploadService(dateTimeSt, radioList[idd1],
                                    context, locale);
                              } else {
                                if ((_image != null && _image.path != null)) {
                                  Toast.show(
                                      locale.pleaseuploadlistText, context,
                                      duration: Toast.LENGTH_SHORT,
                                      gravity: Toast.CENTER);
                                } else if ((addressDelivery != null &&
                                    addressDelivery.addressId != null)) {
                                  Toast.show(
                                      locale.pleaseselectaddressText, context,
                                      duration: Toast.LENGTH_SHORT,
                                      gravity: Toast.CENTER);
                                }
                              }
  ...
  void hitUploadService(
      dateSlot, timeSlot, BuildContext context, AppLocalizations locale) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    int userId = prefs.getInt('user_id');
  ...
  void setVendor() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.setString('vendor_id', widget.vendorId.toString());
  }
}
