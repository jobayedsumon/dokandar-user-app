...
class _ViewCartState extends State<ViewCart> {
  ...
  void createCart(BuildContext context) async {
    if (cartListI != null && cartListI.length > 0) {
      if (cartListI.length <= 5) {
        if (radioList != null && radioList.length > 0) {
          if (totalAmount != null &&
            ...
            if (addressDelivery != null) {
              Toast.show(
                  'Please add something in your cart to proceed!', context,
                  duration: Toast.LENGTH_SHORT);
            } else {
              Toast.show(
        ...
        Toast.show('Can\'t order more than 5 items a time!', context,
            duration: Toast.LENGTH_SHORT);
      }
    } else {
      setState(() {
  ...
  Widget cartOrderItemListTile(
    ...
    return Column(
      children: <Widget>[
        Padding(
            padding: const EdgeInsets.only(left: 4.0, top: 13.3),
            child: ListTile(
              title: Row(
